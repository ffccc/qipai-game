...
<body>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="title">
        <tr>
            ...
            <td width="1232" height="25" valign="top" align="left">目前操作功能：赠送经验</td>
        </tr>
    </table>
    <!-- 头部菜单 End -->
    <% 
        ...
        Sub Save()
            Dim UserArr,i,addExperience,reason,masterID,clientIP
            addExperience = CxGame.GetInfo(1,"in_AddExperience")
            reason = CxGame.GetInfo(0,"in_Reason")
            masterID = session("AdminID")
            clientIP = Request.ServerVariables("REMOTE_ADDR")
            UserArr = Split(Request("userlist"),",")
            For i=0 To UBound(UserArr)
                GameConn.execute("WSP_PM_GrantExperience "&masterID&",'"&clientIP&"',"&UserArr(i)&","&addExperience&",'"&reason&"'")
            Next
            CxGame.MessageBoxClose("赠送经验成功！")
        End Sub
    ...
    <script type="text/javascript">
        function CheckForm()
        {
            var aExperience = document.form1.in_AddExperience.value.trim();
    ...
    <form name="form1" method="post" action='?action=save&userlist=<%=request("UserID") %>' onsubmit="return CheckForm()">
        ...
        <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" class="listBg2">
            <tr>
	            <td class="listTdLeft">赠送经验数目：</td>
	            <td>
	                <input name="in_AddExperience" type="text" class="text" />               
	            </td>
	        </tr>
	        <tr>
