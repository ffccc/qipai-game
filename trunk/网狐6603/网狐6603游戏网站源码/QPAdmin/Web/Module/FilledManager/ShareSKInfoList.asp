...
<%
    If CxGame.GetRoleValue("201",trim(session("AdminName")))<"1" Then
...
<body>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="title">
        <tr>
            ...
            <td width="1232" height="25" valign="top" align="left">你当前位置：充值系统 - 实卡充值</td>
        </tr>
    </table>
    <!-- 头部菜单 End -->
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="titleQueBg Tmg7">
            <tr>
                ...
                <td>
                    ...
                    <input type="button" value="上星期" c
                    lass="btn wd6" onclick="QueryInfo('5')"/>   
    ...
    <%
        ...
        Sub Main()
            ...
            '统计
            Dim rs_1,sql,ArraySum(1,1)
            Set rs_1=Server.CreateObject("Adodb.RecordSet")
            sql = "select Sum(CardPrice) as CardPriceSum ,sum(CardGold) as CardGoldSum from ShareSKDetailInfo where "&queryCondition
        ...
        <div id="content">
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="box Tmg7">
                <tr align="center" class="bold">
                    ...
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('SerialID',this)">销售商</a></th>                   
                ...
                <tr class="<%=className %>" onmouseover="currentcolor=this.style.backgroundColor;this.style.backgroundColor='#caebfc';this.style.cursor='default';"
                    ...
                    <td><%=rs(2,i) %></td>  
                    <td>
                        <a class="l" href="javascript:void(0)" onclick="javascript:openWindowOwn('../AccountManager/AccountsInfo.asp?id=<%=rs(1,i) %>',<%=rs(1,i) %>,850,600)"><%=rs(3,i) %></a>
                    </td>
                    <td><%=rs(4,i) %></td>  
                    <td><a href="javascript:void(0)" onclick="javascript:openWindow('LivcardInfo.asp?SerialID=<%=rs(5,i) %>',700,500)" class="l"><%=rs(5,i) %></a></td>
                    <td><%=CxGame.GetCardNameByCardID(rs(6,i)) %></td>
                    <td><%=FormatNumber(rs(7,i)) %></td>
                    <td><%=FormatNumber(rs(8,i)) %></td>
                    <td><%=FormatNumber(rs(9,i)) %></td>
                    <td><%=CxGame.GetSalesPersonBySerialID(rs(5,i)) %></td>
                    <td><%=rs(10,i) %></td>
                    <td>
