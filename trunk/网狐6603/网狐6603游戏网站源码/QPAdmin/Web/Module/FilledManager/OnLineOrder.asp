...
<%
    If CxGame.GetRoleValue("202",trim(session("AdminName")))<"1" Then
...
<body>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="title">
        <tr>
            ...
            <td width="1232" height="25" valign="top" align="left">你当前位置：充值系统 - 订单管理</td>
        </tr>
    </table>
    <!-- 头部菜单 End -->
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="titleQueBg Tmg7">
            <tr>
                ...
                <td>
                    ...
                    <option value="2"<% If CxGame.GetInfo(1,"swhat")=2 Then %> selected="selected" <%End If %>>按用户帐号</option> 
                    <option value="3"<% If CxGame.GetInfo(1,"swhat")=3 Then %> selected="selected" <%End If %>>按游戏ID</option> 
                    </select>
    ...
    <% 
        ...
        Sub Delete(lID)
            Dim ID
            ID = Trim(lID)
            GameConn.execute "delete OnLineOrder where OnLineID='"&ID&"'"                
        End Sub 
        ...
        Sub Main()
            ...
            End If
            queryCondition = " OnLineID=0"
            Dim field
            field = "OnLineID,UserID,OrderID,ApplyDate,ShareID,Accounts,GameID,CardTypeID,CardPrice,CardTotal,OrderAmount,PayAmount,IPAddress,OperUserID,OrderStatus"
            '查询条件            
        ...
        <div id="content">
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="box">
                <tr align="center" class="bold">
                    ...
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('ApplyDate',this)">订单日期</a></th>   
                    ...
                    <th class="listTitle"><a class="l1"  href="" onclick="GetOrder('OrderStatus',this)">订单状态</a></th>
                </tr>
                <% 
                    Dim className
                    If IsNull(rs) Then
                ...
                <tr class="<%=className %>" onmouseover="currentcolor=this.style.backgroundColor;this.style.backgroundColor='#caebfc';this.style.cursor='default';"
                    ...
                    <td><%=rs(2,i) %></td>
                    <td><%=rs(3,i) %></td>
                    <td><%=CxGame.GetShareNameByShareID(rs(4,i)) %></td>
                    <td>
                        <a class="l" href="javascript:void(0)" onclick="javascript:openWindowOwn('../AccountManager/AccountsInfo.asp?id=<%=rs(1,i) %>',<%=rs(1,i) %>,850,600)"><%=rs(5,i) %></a>
                    </td>
                    <td><%=rs(6,i) %></td>
                    ...
                    <td><%=FormatNumber(rs(11,i)) %></td>
                    <td><%=rs(12,i) %></td>
                    <td>
                        <%If rs(13,i) <> rs(1,i) Then%>
                        <%=CxGame.GetUserNameByUserID(rs(13,i)) %>
                        <%End If %>
                    </td>
                    <td>
                        <% If rs(14,i) = 0 Then %>
                        <span style="color:red">未付款</span>
                        <% Elseif rs(14,i) = 1 Then %>
