...
<%
    If CxGame.GetRoleValue("105",trim(session("AdminName")))<"1" Then
...
<body>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="title">
        <tr>
            ...
            <td width="1232" height="25" valign="top" align="left">你当前位置：游戏用户 - 限制地址</td>
        </tr>
    </table>
    <!-- 头部菜单 End -->
    ...
    <% 
        ...
        Sub SaveConfineInfo()
            ...
            sql = "select * from ConfineAddress where AddrString='"&Request("id")&"'"
            rs.Open sql,GameConn,1,3
            If rs.eof Then
                rs.addnew
                rs("AddrString") = CxGame.GetInfo(0,"in_AddrString")
                '检查重复
                If CxGame.IsExistConfineAddress(Trim(CxGame.GetInfo(0,"in_AddrString"))) Then
                    Call CxGame.ShowInfo2(CxGame.GetInfo(0,"in_AddrString")&"已经存在！","ConfineAddress.asp?action=confineInfo",1200)
                    Exit Sub
                End If
            End If
            ...
            rs.update
            If Request("id")<>"" Then
                Call CxGame.ShowInfo2("修改成功！","ConfineAddress.asp?page="&Request("page")&"",1200)
            Else
                Call CxGame.ShowInfo2("新增成功！","ConfineAddress.asp",1200)
            End If
            rs.close()
            Set rs = Nothing         
        End Sub 
        ...
        Sub Delete(lID)
            Dim ID
            ID = Trim(lID)
            GameConn.execute "delete ConfineAddress where AddrString='"&ID&"'"                
        End Sub
        ...
        Sub Main()
            ...
            If Request("search")<>"" Then
                queryCondition = "AddrString like '%"&CxGame.GetInfo(0,"search")&"%'"
            End If
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="Tmg7">
            <tr>
                <td height="39" class="titleOpBg">
                    <%
                        If trim(session("AdminName"))="admin" or CxGame.GetRoleValue("105",trim(session("AdminName")))="2" or CxGame.GetRoleValue("105",trim(session("AdminName")))="3" Then
                    %>
        ...
        <div id="content">        
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="box">
                <tr class="bold">
                    ...
                    <th class="listTitle2"><a class="l1"  href="" onclick="GetOrder('AddrString',this)">限制地址</a></th>
                ...
                <tr class="<%=className %>" onmouseover="currentcolor=this.style.backgroundColor;this.style.backgroundColor='#caebfc';this.style.cursor='pointer';"
                    ...
                    <td><input name='cid' type='checkbox' value='<%=AddrString%>'/></td>                  
                    <td onclick="Redirect('ConfineAddress.asp?action=confineInfo&id=<%=AddrString %>&page=<%=Request("page") %>')"><%=rs(0,i) %></td>  
                    <td onclick="Redirect('ConfineAddress.asp?action=confineInfo&id=<%=AddrString %>&page=<%=Request("page") %>')">
                        <% If rs(1,i) = 0 Then%>
                        正常
                        <% Else %>
                        禁止
                    ...
                    <td onclick="Redirect('ConfineAddress.asp?action=confineInfo&id=<%=AddrString %>&page=<%=Request("page") %>')"><%=rs(4,i) %></td>    
                    <td onclick="Redirect('ConfineAddress.asp?action=confineInfo&id=<%=AddrString %>&page=<%=Request("page") %>')"><%=rs(5,i) %></td>                
                </tr>
                <% 
                    Next
                    End If
    ...
    <% 
        ...
        Sub GetConfineInfo()
            ...
            Dim addrString,enjoinLogon,enjoinRegister,enjoinOverDate,collectDate,collectNote
            Set rs=Server.CreateObject("Adodb.RecordSet")
            sql = "select * from ConfineAddress where AddrString='"&Request("id")&"'"
            rs.Open sql,GameConn,1,3
            If rs.Bof And rs.Eof Then
                titleInfo = "新增限制ＩＰ地址信息"
            Else
                titleInfo = "修改限制ＩＰ地址信息"
                addrString = rs("AddrString")
    ...
    <script type="text/javascript">
        ...
        {
            var addrString = document.myFormInfo.in_AddrString.value;
            ...
            if(len(addrString)>15)
            {
                alert("地址字符长度不能超过31个字符！");
                return false;
            }      
    ...
    <form name="myFormInfo" method="post" action='?action=saveconfineinfo&id=<%=Request("id") %>&page=<%=Request("page") %>' onsubmit="return CheckFormInfo()">
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
            <tr>
                <td height="39" class="titleOpBg">
                    <input type="button" value="返回" class="btn wd1" onclick="Redirect('ConfineAddress.asp')" />                               
                    <input class="btnLine" type="button" />  
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="listBg2">
            ...
            <tr>
	            ...
	            <td>
	                <% If addrString<>"" Then %>                                       
                    <span><%=addrString %>&nbsp;&nbsp;<%=GetCityFromIP(addrString) %></span>
                    <input name="in_AddrString" type="text" value="<%=addrString %>" style="display:none;" /> 
                    <% Else%>
                    <input name="in_AddrString" type="text" class="text" value="<%=addrString %>" />                    
                    <% End If%>    
	            </td>
	        </tr>
	        <tr>
