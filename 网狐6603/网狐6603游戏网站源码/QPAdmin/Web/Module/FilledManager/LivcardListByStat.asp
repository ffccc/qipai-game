...
<body>
    ...
    <% 
        ...
        Sub Main()
            ...
            If Request("CardTypeID")<>0 Then
                queryCondition = " CardTypeID="&Request("CardTypeID")
            Else
                queryCondition = " 1=1"
            End If
            
            If CxGame.GetInfo(1,"TypeID")<>"" Then
                Select case CxGame.GetInfo(1,"TypeID")
                    case 1
                    case 2
                    ...
                    case 4
                        queryCondition = queryCondition&" And ValidDate<getdate() And ApplyDate is null"
                    case 5
                        queryCondition = queryCondition&" And Nullity=1 And ApplyDate is null"
                End Select
            End If        
            
            Dim field
            field = "CardID,SerialID,CardTypeID,CardPrice,CardGold,MemberOrder,MemberDays,BuildDate,ValidDate,ApplyDate,Nullity,UseRange,SalesPerson"
    ...
    <script type="text/javascript">
        function Operate(opType)
        {
            ...
            document.myformList.action = "?CardTypeID=<%=Request("CardTypeID") %>&TypeID=<%=Request("TypeID") %>&action="+opVal.value;
        ...
        <div id="content">
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="box">
                ...
                <tr class="<%=className %>" onmouseover="currentcolor=this.style.backgroundColor;this.style.backgroundColor='#caebfc';this.style.cursor='default';"
                    ...
                    <td><%=rs(8,i) %></td> 
                    <td><%=rs(9,i) %></td> 
                    <td><%=CxGame.GetCardUseRange(rs(11,i)) %></td>               
                    <td><%=rs(12,i) %></td>                   
                    <td><%=CxGame.GetNullityInfo(rs(10,i)) %></td>                                  
                </tr>
                <% 
                    Next
                    End If
